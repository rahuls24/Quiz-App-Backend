...
export async function saveQuestionsForTheQuiz(
  ...
  try {
    if (
      !(
        Array.isArray(questionsData) && questionsData.every(isValidQuestionData)
      )
    )
      throw createAnError(
...
export async function getAllQuestionsOfAQuiz(
  ...
  try {
    ...
    if (!quizData)
      throw createAnError("Quiz is not found in db", httpStatusCode.notFound);
    ...
    if (user.role === "examinee") {
      if (!quizData.enrolledBy.includes(new Schema.Types.ObjectId(user._id)))
