...
export async function getAllQuizzesForExaminers(
  ...
  if (!examiners.every(isValidMongoObjectId)) {
    let resObj = createFailureResponseObj("Please send a valid examiner id");
    return responseHandler(res, httpStatusCode.badRequest, resObj);
  }
  try {
...
export async function saveQuizStartTime(
  ...
  try {
    ...
    if (!isQuizPresent) throw createAnError("Quiz is not present in DB", 404);
    if (isTimeForCurrentQuizIsPresent)
      throw createAnError("Time is already present", httpStatusCode.conflict);
...
export async function getQuizStartTime(
  ...
  try {
    ...
    if (startTime.length > 1)
      throw createAnError("There is more than one time for this Quiz", 404);
  ...
  try {
    ...
    const quizzesDetails = rawQuizzesDetails.map((quiz) => {
      return {
        ...
        quizResult: getCurrentUserMarks(quiz.marks, currentUser._id.toString())
      };
    });
...
export async function submitQuizHandler(
  ...
  try {
    ...
    if (!isValidSubmittedQuestions(submittedQuestions))
      throw createAnError("Something wrong with submittedQuestions obj", 400);
    ...
    if (!quizTimeDetails)
      throw createAnError("Something went wrong while getting quiz start time");
    let totalTimeTaken = differenceFromNowInMinutes(quizTimeDetails.startedAt);
